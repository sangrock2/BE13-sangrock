package org.example.springfirst.ch05.ex_5_1;

// 서비스 추상화
// 여러가지 구체적인 방법에서 "공통된 개념/동작"만 뽑아내서 하나의 일관된 인터페이스로 표현
// 사용하는 측은 "무엇을 하는지(추상)만 알고, "어떻게 하는지(구현)는 몰라도 된다

// 코드가 특정 기술/환경에 종속되지 않는다
// 기술을 바꾸면 그 기술을 쓰는 코드를 전부 고치지 않아도 된다
// 외부 시스템에 묶이지 않아 테스트에 용이하다.
